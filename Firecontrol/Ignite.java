...
public class Ignite extends SimState{
	...
	public void start(){
		...
		//assign the contractor for every task
		for(Task task : this.tasks){
			task.selectContractor(this);
			System.err.println("contractor task " + task.id +": UAV " + task.contractor.id);
		}
	}
        public int cellsCounter(Ignite ignite, CellType celltype){
                        int cells = 0;
